...
function getRoleState(role) {
  ...
  return gameState.players[role] || null;
}
...
function getMyState() {
  ...
  return getRoleState(clientRole);
}
...
function formatLevelText(player) {
  if (!player || !player.selected) {
    return "-";
  }
  if (player.finishedTraining) {
    return `done ${player.maxLevel}/${player.maxLevel}`;
  }
  return `${player.currentLevel + 1}/${player.maxLevel}`;
}

function formatSequenceText(player) {
  if (!player || !player.selected) {
    return "-";
  }
  if (player.finishedTraining) {
  ...
  return `${player.pickedCount}/${player.targetLength}`;
}
...
function updateRoleUi() {
  ...
  resetSeqBtn.disabled = !isPlayerRole() || quizActive || !myState || myState.finishedTraining;
...
function updateHud() {
  ...
  const blue = getRoleState("blue");
  const yellow = getRoleState("yellow");
  ...
  levelValue.textContent = `Blue L ${formatLevelText(blue)} | Yellow L ${formatLevelText(yellow)}`;
  seqValue.textContent = `Blue Seq ${formatSequenceText(blue)} | Yellow Seq ${formatSequenceText(yellow)}`;

  const myState = getMyState();
  speedValue.textContent = myState ? String(myState.speed) : "-";
...
function drawScene(now, world) {
  ...
  if (gameState && gameState.message && now < gameState.messageUntil) {
    ...
    roundRect(ctx, world.width / 2 - 280, 32, 560, 56, 14, true, false);
    ctx.fillStyle = "#2341a3";
    roundRect(ctx, world.width / 2 - 284, 28, 560, 56, 14, true, false);
...
function drawBottomPanel(state, world) {
  ...
  ctx.fillText(
    ...
    world.width / 2 - 200,
    world.height - world.panelHeight + 30,
  );
  ctx.fillText(
    `Yellow: lives ${yellow.lives} speed ${yellow.speed} level ${formatLevelText(yellow)}`,
    world.width / 2 - 200,
    world.height - world.panelHeight + 56,
  );

  const blueSentence = (blue.levelDescription || []).join(" ");
  const yellowSentence = (yellow.levelDescription || []).join(" ");
  ctx.font = "700 17px Manrope";
  ctx.fillText(`Blue words: ${blueSentence}`, world.width / 2 - 200, world.height - world.panelHeight + 84);
  ctx.fillText(`Yellow words: ${yellowSentence}`, world.width / 2 - 200, world.height - world.panelHeight + 108);
}
...
function render() {
  ...
  if (state.quiz?.active) {
    ...
    drawOverlayText(world, "TRAINING DONE", "Both players completed all levels.");
